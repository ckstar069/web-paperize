...
const PROBE = `(async () => {
  ...
  const out = {
    ...
    sourceUrlVisible: doc ? [...doc.querySelectorAll('.sources .src')].some((r) => r.textContent.includes('https://example.com/src')) : false,
...
try {
  ...
  if (r.exceptionDetails) { console.log('EXC', JSON.stringify(r.exceptionDetails).slice(0, 400)); failed = 1; }
    ...
    ['sources URL visible as text', v.sourceUrlVisible],
